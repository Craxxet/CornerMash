export class RatingsDO {
  ...
  // CORS preflight: any OPTIONS request to any path is answered here.
  // The browser sends this automatically before any "non-simple" cross-origin
  // request (custom headers, JSON body, etc.). Today the reset call is a simple
  // POST so no preflight fires — but handle it anyway so future code that
  // uses headers doesn't silently break.
  if (request.method === "OPTIONS") {
    return new Response(null, {
      status: 204,
      headers: {
        "Access-Control-Allow-Origin": "https://cornermash.pages.dev",
  ...
  if (provided !== this.env.RESET_SECRET) {
    return new Response("Forbidden", {
      status: 403,
      headers: { "Access-Control-Allow-Origin": "https://cornermash.pages.dev" },
    });
  }
  ...
  return Response.json(
    { ok: true, message: "DO storage cleared" },
    { headers: { "Access-Control-Allow-Origin": "https://cornermash.pages.dev" } }
  );
}
}
